...
\section{Background Research}\label{resarch}
	...
	However, Operating Systems at different interconnections distances is a concern, as our sponsor has a special requirement of no WIFI at certian times when the application is operating. So that is one issue that has to be accomadated. Another issue is going to be the storage on the Nexus 7 tablets. A usual tablet holds up to about 16GB of memory, so we need to make sure that our application is well within that limit.  \\
